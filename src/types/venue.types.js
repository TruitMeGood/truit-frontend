export const FETCH_VENUE = 'FETCH_VENUE';
export const FETCH_VENUE_SUCCESS = 'FETCH_VENUE_SUCCESS';
export const FETCH_VENUE_ERROR = 'FETCH_VENUE_ERROR';

export const SET_VENUE = 'SET_VENUE';

export const FETCH_INSTAGRAM = 'FETCH_INSTAGRAM';
export const FETCH_INSTAGRAM_SUCCESS = 'FETCH_INSTAGRAM_SUCCESS';
export const FETCH_INSTAGRAM_ERROR = 'FETCH_INSTAGRAM_ERROR';

export const NEARBY_VENUES_VISIBLE = 'NEARBY_VENUES_VISIBLE';

export const FETCH_FOURSQUARE = 'FETCH_FOURSQUARE';
export const FETCH_FOURSQUARE_SUCCESS = 'FETCH_FOURSQUARE_SUCCESS';
export const FETCH_FOURSQUARE_ERROR = 'FETCH_FOURSQUARE_ERROR';
